...
class DataTypes(enum.IntEnum):
    ...
    REAL_VALUED = 0 #实数型变量（连续值）
    CATEGORICAL = 1 #分类变量（离散值）
    DATE = 2 #日期或时间型变量
...
class InputTypes(enum.IntEnum):
    ...
    TARGET = 0 #预测目标（模型输出）
    OBSERVED_INPUT = 1 #观测输入：仅在当前或历史已知
    KNOWN_INPUT = 2 #已知输入：未来时间也可以提前知道
    STATIC_INPUT = 3 #静态输入：固定不变的输入
    ID = 4 #实体唯一标识
    TIME = 5 #时间索引
...
def extract_cols_from_data_type(data_type, column_definition, excluded_input_types):
    """根据指定的数据类型（data_type）筛选出列定义中对应的列名，但排除掉一些指定输入类型的列
...
class ModelFeatures:
    ...
    def __init__(
        ...
        """初始化格式化程序。将数据帧拆分为训练-验证-测试数据帧。也校准缩放对象，并为每个分割转换数据"""
